package fr.gsb.rv.fr.gsb.rv.modeles;
...
import fr.gsb.rv.fr.gsb.rv.entites.Praticien;
import fr.gsb.rv.fr.gsb.rv.entites.Motif;
...
public class ModeleGsb {
	...
	private void peupler(){
		...
		this.lesRapportsVisites.get(0).setLeMotif(this.lesMotifs.get(0)) ;
		this.lesRapportsVisites.get(0).setLePraticien(this.lesPraticiens.get(4)) ;
	...
	public List<RapportVisite> getRapportsVisites(Visiteur visiteur, int mois, int annee){
		...
		for(RapportVisite unRapport : this.lesRapportsVisites){
			...
			//if(unRapport.getLeVisiteur().getMatricule() == visiteur.getMatricule()){
